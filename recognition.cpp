...
void showTrainingDebugData(const Ptr<FaceRecognizer> model, const int faceWidth, const int faceHeight, const string algorithm)
{
    try {   // Surround the OpenCV calls by a try/catch block so we don't crash if some model parameters aren't available.
		...
		imwrite("./immagini/average.jpg", averageFace);
        ...
        for (int i = 0; i < min(20, eigenvectors.cols); i++) {
			...
			if (algorithm == "FaceRecognizer.Fisherfaces"){
				string imgName = "fisherface" + to_string(i) + ".jpg";
				string completePath = path + imgName;
				imwrite(completePath, eigenface);
			}
			else {
				string imgName = "eigenface" + to_string(i) + ".jpg";
				string completePath = path + imgName;
				imwrite(completePath, eigenface);
			}
        }
